...
const Login: React.FC = () => {
  ...
  // Sincronização Final: O Professor exato para esta turma
      ...
      <div className="min-h-screen flex items-center justify-center bg-gray-100 p-4">
          <div className="bg-white p-8 rounded-3xl shadow-xl w-full max-w-md border-b-4 border-gray-200 text-center">
              ...
              <div className="space-y-4">
                  <button onClick={() => handleRoleSelect(UserRole.STUDENT)} className="w-full p-6 rounded-2xl border-2 border-primary bg-blue-50 text-primary hover:bg-blue-100 transition-all flex items-center gap-4 group">
                  ...
                  <button onClick={() => handleRoleSelect(UserRole.TEACHER)} className="w-full p-6 rounded-2xl border-2 border-secondary bg-green-50 text-secondary hover:bg-green-100 transition-all flex items-center gap-4 group">
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-100 p-4">
          <div className="bg-white p-8 rounded-3xl shadow-xl w-full max-w-md border-b-4 border-gray-200 text-center">
              ...
              <h2 className="text-2xl font-extrabold text-gray-800 mb-6">{role === UserRole.STUDENT ? 'Área do Aluno' : 'Área do Professor'}</h2>
              <div className="flex gap-4 mb-6">
                  <button onClick={() => setMode('login')} className={`flex-1 py-3 font-bold rounded-xl ${mode === 'login' ? 'bg-primary text-white' : 'bg-gray-100 text-gray-500'}`}>ENTRAR</button>
                  {role !== UserRole.ADMIN && (
                      <button onClick={() => setMode('register')} className={`flex-1 py-3 font-bold rounded-xl ${mode === 'register' ? 'bg-secondary text-white' : 'bg-gray-100 text-gray-500'}`}>CADASTRAR</button>
                  )}
              </div>
              {mode === 'login' ? (
                  <form onSubmit={handleLoginSubmit} className="space-y-4">
                      <div className="text-left">
                          ...
                          <input type="text" value={email} onChange={e => setEmail(e.target.value)} className="w-full p-3 rounded-xl border-2 border-gray-200 font-bold outline-none" required />
                      </div>
                      {role === UserRole.ADMIN && (
                          <div className="text-left">
                              <label className="block text-xs font-black text-gray-400 uppercase mb-1">SENHA</label>
                              <input type="password" value={password} onChange={e => setPassword(e.target.value)} className="w-full p-3 rounded-xl border-2 border-gray-200 font-bold outline-none" required />
                          </div>
                      )}
                      {errorMsg && <p className="text-red-500 text-xs font-bold">{errorMsg}</p>}
                      <button type="submit" disabled={isLoading} className="w-full bg-primary text-white font-black py-3 rounded-xl shadow-lg border-b-4 border-blue-700">{isLoading ? 'CARREGANDO...' : 'ACESSAR'}</button>
                  </form>
              ) : (
                  <button onClick={() => setStep(3)} className="w-full bg-secondary text-white font-black py-3 rounded-xl shadow-lg border-b-4 border-green-700">INICIAR CADASTRO</button>
              )}
          </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-100 p-4">
      <div className="bg-white p-8 rounded-3xl shadow-xl w-full max-w-md border-b-4 border-gray-200 max-h-[90vh] overflow-y-auto custom-scrollbar">
        <h2 className="text-2xl font-extrabold text-gray-800 mb-6 text-center">Cadastro de {role === UserRole.STUDENT ? 'Aluno' : 'Professor'}</h2>
        <form onSubmit={handleRegisterSubmit} className="space-y-4">
            ...
            <div>
                 ...
                 <select required disabled={!city} value={school} onChange={(e) => setSchool(e.target.value)} className="w-full p-3 rounded-xl border-2 border-gray-200 font-bold bg-white mb-2">
                    ...
                    {role === UserRole.TEACHER && <option value="other">+ Cadastrar Nova</option>}
                 </select>
                 {school === 'other' && <input required type="text" value={customSchool} onChange={(e) => setCustomSchool(e.target.value)} placeholder="Nome da escola" className="w-full p-3 rounded-xl border-2 border-secondary font-bold" />}
            </div>
            ...
            {role === UserRole.STUDENT && (
                <div className={`p-4 rounded-2xl border-2 transition-all ${matchingTeacher ? 'bg-green-50 border-secondary' : 'bg-gray-50 border-gray-100'}`}>
                    <label className="block text-primary font-black text-xs uppercase mb-1">Sincronização Docente</label>
                    {matchingTeacher ? (
                        <p className="font-bold text-secondary flex items-center gap-2"><CheckCircle size={16} /> Prof. {matchingTeacher.name}</p>
                    ) : (
                        <p className="text-gray-400 italic text-xs">Aguardando localidade/turma...</p>
                    )}
                </div>
            )}
            <div className="flex gap-4 pt-4">
                <button type="button" onClick={() => setStep(2)} className="px-6 py-3 font-bold text-gray-400">Voltar</button>
                <button type="submit" disabled={role === UserRole.STUDENT && !matchingTeacher} className="flex-1 bg-primary text-white font-black py-3 rounded-xl shadow-lg border-b-4 border-blue-700 disabled:opacity-50">CONCLUIR</button>
            </div>
        </form>
      </div>
    </div>
